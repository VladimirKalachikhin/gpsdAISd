...
do {
	...
	case 'AIS':
		...
		case 24: 	// Vendor ID не поддерживается http://www.e-navigation.nl/content/static-data-report
			//echo "JSON gpsdData: \n"; print_r($gpsdData); echo "\n";
			if($gpsdData['imo']) $aisData[$vehicle]['imo'] = (string)$gpsdData['imo']; 	// IMO number 0 = not available = default – Not applicable to SAR aircraft 0000000001-0000999999 not used 0001000000-0009999999 = valid IMO number; 0010000000-1073741823 = official flag state number.
			if($gpsdData['ais_version']) $aisData[$vehicle]['ais_version'] = (int)filter_var($gpsdData['ais_version'],FILTER_SANITIZE_NUMBER_INT); 	// AIS version indicator 0 = station compliant with Recommendation ITU-R M.1371-1; 1 = station compliant with Recommendation ITU-R M.1371-3 (or later); 2 = station compliant with Recommendation ITU-R M.1371-5 (or later); 3 = station compliant with future editions
			if($gpsdData['callsign']=='@@@@@@@') $aisData[$vehicle]['callsign'] = NULL;
			elseif($gpsdData['callsign']) $aisData[$vehicle]['callsign'] = (string)$gpsdData['callsign']; 	// Call sign 7 x 6 bit ASCII characters, @@@@@@@ = not available = default. Craft associated with a parent vessel, should use “A” followed by the last 6 digits of the MMSI of the parent vessel. Examples of these craft include towed vessels, rescue boats, tenders, lifeboats and liferafts.
			...
			if($gpsdData['shiptype']) $aisData[$vehicle]['shiptype'] = (int)filter_var($gpsdData['shiptype'],FILTER_SANITIZE_NUMBER_INT); 	// Type of ship and cargo type 0 = not available or no ship = default 1-99 = as defined in § 3.3.2 100-199 = reserved, for regional use 200-255 = reserved, for future use Not applicable to SAR aircraft
			if($gpsdData['shiptype_text']) $aisData[$vehicle]['shiptype_text'] = filter_var($gpsdData['shiptype_text'],FILTER_SANITIZE_STRING); 	// 
			if($gpsdData['to_bow']) $aisData[$vehicle]['to_bow'] = (float)filter_var($gpsdData['to_bow'],FILTER_SANITIZE_NUMBER_FLOAT); 	// Reference point for reported position. Also indicates the dimension of ship (m) (see Fig. 42 and § 3.3.3) For SAR aircraft, the use of this field may be decided by the responsible administration. If used it should indicate the maximum dimensions of the craft. As default should A = B = C = D be set to “0”
			if($gpsdData['to_stern']) $aisData[$vehicle]['to_stern'] = (float)filter_var($gpsdData['to_stern'],FILTER_SANITIZE_NUMBER_FLOAT); 	// Reference point for reported position.
			if($gpsdData['to_port']) $aisData[$vehicle]['to_port'] = (float)filter_var($gpsdData['to_port'],FILTER_SANITIZE_NUMBER_FLOAT); 	// Reference point for reported position.
			if($gpsdData['to_starboard']) $aisData[$vehicle]['to_starboard'] = (float)filter_var($gpsdData['to_starboard'],FILTER_SANITIZE_NUMBER_FLOAT); 	// Reference point for reported position.
		...
		case 8: 	// 
			//echo "JSON gpsdData:\n"; print_r($gpsdData); echo "\n";
			$aisData[$vehicle]['dac'] = (string)$gpsdData['dac']; 	// Designated Area Code
			$aisData[$vehicle]['fid'] = (string)$gpsdData['fid']; 	// Functional ID
			if($gpsdData['vin']) $aisData[$vehicle]['vin'] = (string)$gpsdData['vin']; 	// European Vessel ID
			if($gpsdData['length']) $aisData[$vehicle]['length'] = (float)filter_var($gpsdData['length'],FILTER_SANITIZE_NUMBER_INT)/10; 	// Length of ship in m
			if($gpsdData['beam']) $aisData[$vehicle]['beam'] = (float)filter_var($gpsdData['beam'],FILTER_SANITIZE_NUMBER_INT)/10; 	// Beam of ship in m
			if(!$aisData[$vehicle]['shiptype']) $aisData[$vehicle]['shiptype'] = (string)$gpsdData['shiptype']; 	// Ship/combination type ERI Classification В какой из посылок тип правильный - неизвестно, поэтому будем брать только из одной
			if(!$aisData[$vehicle]['shiptype_text'])$aisData[$vehicle]['shiptype_text'] = filter_var($gpsdData['shiptype_text'],FILTER_SANITIZE_STRING); 	// 
			$aisData[$vehicle]['hazard'] = (int)filter_var($gpsdData['hazard'],FILTER_SANITIZE_NUMBER_INT); 	// Hazardous cargo | 0 | 0 blue cones/lights | 1 | 1 blue cone/light | 2 | 2 blue cones/lights | 3 | 3 blue cones/lights | 4 | 4 B-Flag | 5 | Unknown (default)
			$aisData[$vehicle]['hazard_text'] = filter_var($gpsdData['hazard_text'],FILTER_SANITIZE_STRING); 	// 
			if(!$aisData[$vehicle]['draught']) $aisData[$vehicle]['draught'] = (float)filter_var($gpsdData['draught'],FILTER_SANITIZE_NUMBER_INT)/100; 	// Draught in m ( 1-200 * 0.01m, default 0)
			$aisData[$vehicle]['loaded'] = (int)filter_var($gpsdData['loaded'],FILTER_SANITIZE_NUMBER_INT); 	// Loaded/Unloaded | 0 | N/A (default) | 1 | Unloaded | 2 | Loaded
			$aisData[$vehicle]['loaded_text'] = filter_var($gpsdData['loaded_text'],FILTER_SANITIZE_STRING); 	// 
			$aisData[$vehicle]['speed_q'] = (int)filter_var($gpsdData['speed_q'],FILTER_SANITIZE_NUMBER_INT); 	// Speed inf. quality 0 = low/GNSS (default) 1 = high
			$aisData[$vehicle]['course_q'] = (int)filter_var($gpsdData['course_q'],FILTER_SANITIZE_NUMBER_INT); 	// Course inf. quality 0 = low/GNSS (default) 1 = high
			$aisData[$vehicle]['heading_q'] = (int)filter_var($gpsdData['heading_q'],FILTER_SANITIZE_NUMBER_INT); 	// Heading inf. quality 0 = low/GNSS (default) 1 = high
			break;
		}
	}
	...
	if($aisDataOld) {
	    ...
	    $aisDataPreserve = array_diff_key($aisDataOld,$aisData); 	// цели в файле, которых нет в текущем наборе целей
		//echo "\n aisDataPreserve "; print_r($aisDataPreserve);
	    //echo "\n aisData "; print_r($aisData);
	    $aisData = $aisData+$aisDataPreserve; 	// array_merge перенумеровывает числовые ключи
	...
	if((time()-$aisVatch) >= $noVehicleTimeout) { 	// checking visible of ships periodically
		...
		$bf = count($aisData);
		$aisData = array_filter($aisData,'chkVe',ARRAY_FILTER_USE_KEY); 	// оставим в $aisData только те цели, которые есть в $aisVehicles
...
return ((time()-$aisData[$vhID]['timestamp']>$noVehicleTimeout) or in_array($vhID,$aisVehicles));
...
if(file_exists($aisJSONfileName)) { 	// однако, файл с целями AIS есть.
	...
	//print_r($aisData);
	$ch = FALSE;
	foreach($aisData as $vehicle => &$data) { 	// почистим файл от старых целей
		if((time()-$data['timestamp'])>$noVehicleTimeout) {
			//echo time()-$data['timestamp']."\n";
			unset($aisData[$vehicle]);
			$ch = TRUE;
		}
	}
	if($ch) {
